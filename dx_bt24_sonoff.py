...
EW_REGION = "eu"  # cambia se il tuo account non è EU
...
async def main():
    ...
    # -------------------------
    # Connessione SonoffBasic cloud
    # -------------------------
    sonoff = Sonoff(EW_EMAIL, EW_PASSWORD, EW_REGION)
    print("✅ Connesso a eWeLink cloud tramite SonoffBasic")

    # Funzioni per accendere / spegnere la luce
    async def luce_on():
        sonoff.change_device_status(deviceid=DEVICE_ID, new_status="on")
        print("💡 Luce ACCESA")

    async def luce_off():
        sonoff.change_device_status(deviceid=DEVICE_ID, new_status="off")
        print("🌑 Luce SPENTA")
